...
function analyzeSourceCode(sourceContent) {
    const functions = [];
    const classes = [];
    const exports = [];
    // 清理代码，移除注释和字符串
...
function generateSmartTestContent(fileName, importPath, analysis) {
...
function generateFunctionTests(func) {
...
function generateClassTests(cls) {
    let content = `\n  describe('${cls.name}', () => {\n`;
    // 类存在性测试
    content += `    it('should be defined', () => {\n`;
...
function generateBasicModuleTest(fileName, importPath) {
    return `
...
function isSideEffectFunction(func) {
...
function determineReturnAssertion(func) {
    const returnType = func.returnType?.toLowerCase();
    const code = func.sourceCode.toLowerCase();
    ...
    // 分析返回值的模式
    const returnValues = returnMatches.map(stmt => stmt.replace('return', '').trim().replace(';', ''));
    // 检查返回值类型模式
    const hasStringReturn = returnValues.some(val => val.startsWith('"') || val.startsWith("'") || val.startsWith('`') ||
        val.includes('.toString()') || val.includes('string'));
    const hasNumberReturn = returnValues.some(val => /^\d+$/.test(val) || val.includes('math.') || val.includes('number') ||
        /[+\-*/]/.test(val) && !/['"`]/.test(val));
    const hasBooleanReturn = returnValues.some(val => val === 'true' || val === 'false' || val.includes('boolean') ||
        /===|!==|==|!=|<|>|<=|>=/.test(val));
    const hasArrayReturn = returnValues.some(val => val.startsWith('[') || val.includes('.map(') || val.includes('.filter(') ||
        val.includes('array') || val.includes('[]'));
    const hasObjectReturn = returnValues.some(val => val.startsWith('{') || val.includes('object') || val.includes('new '));
    // 根据检测到的模式返回断言
    if (hasStringReturn) {
        return "expect(typeof result).toBe('string');";
    }
    ...
    const hasActualReturn = returnValues.some(val => val && val !== 'undefined' && val !== 'null' && val !== '""' && val !== "''");
    if (hasActualReturn) {
        return "expect(result).toBeDefined();";
    }
    // 4. 默认情况：可能是副作用函数
